...
class BaseLlmFlow(ABC):
  ...
  async def _postprocess_async(
    ...
    if model_response_event.get_function_calls():
      ...
      # Handle fallback_to_parent if no function calls occurred
      async for event in self._handle_fallback_to_parent_async(
          invocation_context, model_response_event
      ):
        yield event
  ...
  async def _handle_fallback_to_parent_async(
      self,
      invocation_context: InvocationContext,
      model_response_event: Event,
  ) -> AsyncGenerator[Event, None]:
    """Handle fallback_to_parent logic when no function calls occurred.
    ...
    - parent_agent exists
    - The model response contains no function calls
    ...
    Yields:
      Events generated by the fallback transfer if applicable.
    """
    from ...agents.llm_agent import LlmAgent

    agent = invocation_context.agent

    # Check all conditions for fallback
    if (
      ...
      transfer_call = Part.from_function_call(
          name='transfer_to_agent', args={'agent_name': agent.parent_agent.name}
      )

      # Create transfer event with function call
      transfer_event = Event(
          id=Event.new_id(),
          invocation_id=invocation_context.invocation_id,
          author=agent.name,
      )
      transfer_event.content = types.Content(
          role='model', parts=[transfer_call]
      )
      yield transfer_event

      # Create function response event
      transfer_response = Part.from_function_response(
          name='transfer_to_agent', response={'result': None}
      )
      transfer_response_event = Event(
          id=Event.new_id(),
          invocation_id=invocation_context.invocation_id,
          author=agent.name,
      )
      transfer_response_event.content = types.Content(
          role='model', parts=[transfer_response]
      )
      transfer_response_event.actions.transfer_to_agent = (
          agent.parent_agent.name
      )
      yield transfer_response_event

      # Execute the parent agent
      async for event in agent.parent_agent.run_async(invocation_context):
        yield event
